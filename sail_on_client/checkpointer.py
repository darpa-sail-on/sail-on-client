...
class Checkpointer(object):
    ...
    def _save_elementwise_attribute(self, detector: Any, attribute: str,
            attribute_dict: Dict) -> Dict:
        """
        ...
        if isinstance(attribute_val, dict):
            ...
            if test_id in attribute_dict:
                attribute_dict[test_id] = torch.cat([attribute_dict[test_id],
                                                     attribute_val])
            else:
                attribute_dict[test_id] = attribute_val
        else:
            logging.info("Treating attribute value as a single element rather than an iterable")
            attribute_dict[test_id] = attribute_val
        ...
        if len( attributes )>0:
            for attribute in attributes:
                if hasattr(self.detector, attribute) and save_elementwise:
                    ...
                    attribute_dict[attribute] = \
                            self._save_elementwise_attribute(self.detector,
                                                             attribute,
                                                             attribute_dict[attribute])
                elif hasattr(self.detector, attribute) and not save_elementwise:
                    raise NotImplementedError("Saving attributes for an entire round is not supported")
                else:
    ...
    def _restore_elementwise_attribute(self, detector: Any, attribute_name: str,
            attribute_val: Dict) -> Any:
        """
        ...
        """
        dataset_ids = list(map(lambda x: x.strip(),
                               open(self.toolset['dataset'], "r").readlines()))
        ...
        elif isinstance(attribute_val, list) or \
             isinstance(attribute_val, tuple) or isinstance(attribute_val, Tensor):
            round_attribute_val = attribute_val[round_id*round_len:(round_id+1)*round_len]
        else:
            logging.info("Treating attribute value as a single element rather than an iterable")
    ...
    def restore_attributes(self, step_descriptor):
        ...
        :return None
        """
        if self.toolset["use_saved_attributes"] and \
                step_descriptor in self.toolset["saved_attributes"]:
            ...
            for attribute in attributes:
                if save_elementwise:
                    self.detector = \
                            self._restore_elementwise_attribute(self.detector,
                                                                attribute,
                                                                attribute_val[attribute][test_id])
                else:
                    raise NotImplementedError("Restoring attributes for an entire round is not supported")
